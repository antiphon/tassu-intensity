---
title: "Wolf observations, rough intensity vol. 2.61 (github)"
...
* v2.61: github version; wolf-year
...
Data frame prepared in `1-aggregate-model-data-frame-v1.R`.
...
Tassu on map, spatial

```{r, fig.height=7, fig.width=9}
p <- dat2 %>% 
  ggplot() + 
  geom_sf(data = fin1, fill=NA) + 
  geom_sf( data = gps, aes(fill = type), alpha=.3, col = NA)+
  geom_point(aes(x,y), alpha=0.2, size = 0.2) + 
  facet_grid(.~lajis)+
  ggtitle("Tassu| GPS+expert territories ")  
...
p1 <- nsusi ~ -1  +  vuosif + kkf  + special_may + ti(x, y) + offset(-log(area_km2))
...
them <- !is.na(c20s$Tiet[])
c18df <- data.frame(c18=as.data.frame(c20s), 
                    as.data.frame(droad1),
                    vuosif = "2011", kkf = "1", special_may = FALSE, # need for predict-method
                    area_km2 = median(model_dat$area_km2),
                    areasum = median(model_dat$areasum)
                    ) %>% 
  bind_cols( coordinates(c20s) %>% data.frame() )
c18df <- c18df[them,]
...
pred_s <- predict.bam(fitbest, c18df, terms = "ti(x,y)", 
                      type = "terms", 
                      se.fit = TRUE, 
                      cluster = cl) 

lammap_smooth <- data.frame(
  log_lam = c(pred_s$fit),
  log_lam.se = c(pred_s$se.fit), term = "ti(x,y)") %>%
...
# and together
lammap <- data.frame(
  log_lam = rowSums(pred_corine$fit) + c(pred_s$fit),
  log_lam.se = sqrt( rowSums(pred_corine$se.fit^2) + c(pred_s$se.fit)^2 ), 
  term = "ti(x,y) + corine[all 7]") %>%
  bind_cols(c18df %>% dplyr::select(x, y) ) 
...
lammaps %>% filter(term == "ti(x,y)") %>% mapit()
```
...
saveRDS(out, "poisson_regression_log_lambda_terms-v2.6.rds")

# store now also the model object
saveRDS(pit1, "poisson_regression_bamfit-v2.6.rds")
